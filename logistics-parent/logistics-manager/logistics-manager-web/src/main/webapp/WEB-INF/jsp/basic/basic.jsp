        ...
        <c:forEach items="${list}" var="data">
            ...
            <c:if test="${data.parentId eq null}">
                <tr>
                    ...
                    <td>${data.parentId}</td>
                    <td>
                        <a href="/basic/basicDispatch?id=${data.baseId}" class="tablelink">更新</a>
                        <a href="javascript:void(0)" onclick="deleteRole(${role.roleId})" class="tablelink">删除</a>
                    </td>
                </tr>
                <c:forEach items="${list}" var="small">
                    <c:if test="${small.parentId eq data.baseId}">
                        <tr>
                            <td><input name="" type="checkbox" value="" /></td>
                            <td>${small.baseId}</td>
                            <td style="padding-left: 40px;">${small.baseName}</td>
                            <td style="padding-left: 40px;">${small.baseDesc}</td>
                            <td>${small.parentId}</td>
                            <td>
                                <a href="/basic/basicDispatch?id=${small.baseId}" class="tablelink">更新</a>
                                <a href="javascript:void(0)" onclick="deleteRole(${role.roleId})" class="tablelink">删除</a>
                            </td>
                        </tr>
                    </c:if>
